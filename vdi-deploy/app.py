...
def batch_add():
    ...
    if not f: flash("请上传CSV","danger"); return redirect(url_for("index"))
    r=csv.DictReader(io.StringIO(f.stream.read().decode("UTF-8"))); pm=PM(); sc=0; el=[]; db=DB()
    for row in r:
        try:
            gu=row["guac_username"].strip(); gp=row["guac_password"].strip(); vu=row["vm_username"].strip(); vp=row["vm_password"].strip()
            nm=row["vmname"].strip(); vi=int(row["vmid"]); ti=int(row["template"])
            cm=row.get("clone_mode","full").strip(); fu=1 if cm=="full" else 0
            ip=row.get("manual_ip","").strip() if row.get("ip_mode","auto")=="manual" else None
            if not ip: ip=pm.get_vm_ip(vi,180)
            pm.clone_vm(ti,vi,nm,None,fu)
            if not ip: el.append(f"{nm}:IP失败"); continue
            ei=db.add_user(gu,gp); ci=db.add_connection(nm,ip,vu,vp,vi,row.get("security_mode","any")); db.authorize(ei,ci); db.c.commit(); sc+=1
        except Exception as ex: el.append(f"{nm}:{ex}")
    db.c.commit(); db.close()
    flash(f"批量完成 成功:{sc} 失败:{len(el)}","info" if not el else "warning")
    for e in el[:5]: flash(f"  - {e}","warning")
    return redirect(url_for("index"))

@app.route("/delete",methods=["POST"])
@login_required
def delete():
    nm=request.form["vmname"]; vi=int(request.form["vmid"]); pm=PM(); db=DB(); ds=db.list_desktops()
    t=next((d for d in ds if d["connection_name"]==nm),None)
    if t: db.remove_connection(t["connection_id"]); u=t.get("username"); ei=db.get_entity(u) if u else None
    if ei: db.remove_user(ei,u)
    db.close(); pm.destroy_vm(vi); flash(f"{nm} 已删除","success"); return redirect(url_for("index"))
...
def user():
    if request.method=="POST":
        ...
        if a=="login":
            u=request.form["username"].strip(); p=request.form["password"].strip(); db=DB()
            if db.validate_user_password(u,p): session["user"]=u; db.close(); return redirect(url_for("user"))
            ...
            if np:
                db=DB(); ei=db.get_entity(session["user"])
                if ei:
                    s,h=db._hash(np); cur=db.c.cursor()
                    cur.execute("UPDATE guacamole_user SET password_salt=UNHEX(%s),password_hash=UNHEX(%s),password_date=NOW() WHERE entity_id=%s",(s,h,ei)); db.c.commit()
                db.close(); flash("密码修改成功","success")
            return redirect(url_for("user"))
        elif a=="logout": session.pop("user",None); return redirect(url_for("user"))
    if "user" not in session: return render_template("user_login.html")
    u=session["user"]; pm=PM(); db=DB(); cur=db.c.cursor(dictionary=True)
    cur.execute("SELECT c.connection_id,c.connection_name,host.parameter_value ip,vmid.parameter_value vmid FROM guacamole_connection c JOIN guacamole_connection_permission p ON c.connection_id=p.connection_id JOIN guacamole_entity e ON p.entity_id=e.entity_id LEFT JOIN guacamole_connection_parameter host ON c.connection_id=host.connection_id AND host.parameter_name='hostname' LEFT JOIN guacamole_connection_parameter vmid ON c.connection_id=vmid.connection_id AND vmid.parameter_name='vmid' WHERE e.name=%s",(u,))
    des=cur.fetchall(); cur.close(); db.close()
    for d in des:
        if d.get("vmid"):
            try: d["status"]=pm.get_vm_status(int(d["vmid"]))
            except: d["status"]="unknown"
    return render_template("user_desktop.html",username=u,desktops=des,guac_config=CONFIG["guacamole"])
